...
# ]
# ///
"""Update air-pre-commit to the latest version of air."""
...
def get_all_versions() -> list[Version]:
    response = urllib3.request("GET", "https://pypi.org/pypi/air-formatter/json")
...
def get_current_version(pyproject: dict) -> Version:
    ...
    requirement = next((r for r in requirements if r.name == "air-formatter"), None)
    assert requirement is not None, "pyproject.toml does not have air-formatter requirement"

    specifiers = list(requirement.specifier)
    assert (
        len(specifiers) == 1 and specifiers[0].operator == "=="
    ), f"air's specifier should be exact matching, but `{requirement}`"
...
def process_version(version: Version) -> typing.Sequence[str]:
    def replace_pyproject_toml(content: str) -> str:
        return re.sub(r'"air-formatter==.*"', f'"air-formatter=={version}"', content)
